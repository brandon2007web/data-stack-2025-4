...
<div class="dashboard">
 ...
 <table class="horario-table">
    <thead>
        <tr>
            <th class="hora-header" colspan="2">Hora</th>
    ...
    <tbody>
        <?php foreach ($horas as $hora): ?>
            <tr>
                ...
                <th class="hora-header"><?php echo htmlspecialchars($hora['Rango']); ?></th>
                <?php foreach ($dias as $dia):
                    ...
                    // Si el horario ya fue marcado
                    if (isset($marcados[$idDia][$idHora])) $clase .= ' seleccionado';

                    // 🔹 Ahora TODAS las celdas son editables si tiene permiso
                    if ($tienePermisoEdicion) {
                        $clase .= ' editable';
                        $dataAttributes = "data-grupo='$grupoSeleccionadoID' data-id-dia='$idDia' data-id-hora='$idHora'";
                        // Si no está libre, agregamos el tooltip
                        if ($asignatura !== 'Libre') {
                            $clase .= ' tooltip';
                            $dataAttributes .= " data-tooltip='" . htmlspecialchars($asignatura) . "'";
                        }
                    }
                ?>
        ...
        <?php else: ?>
            <p style="text-align:center; margin-top:20px;">Seleccione un grupo para ver su horario.</p>
        <?php endif; ?>
    </div>
</div>
